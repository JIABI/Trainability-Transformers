...
class DropPath(nn.Module):
    ...
    def __init__(self, drop_prob: float = 0.0): super().__init__(); self.drop_prob = float(drop_prob)
...
class PerformerAttention(nn.Module):
    ...
    def _maybe_init_proj(self, device):
        ...
        if self.proj_matrix is None or self.proj_matrix.device != device:
            ...
            for _ in range(self.num_heads):
                W = gaussian_orthogonal_random_matrix(self.head_dim, self.m_features, device=device,
                                                      dtype=torch.float32)
    ...
    def forward(self, x: torch.Tensor, attn_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        if attn_mask is not None:
            # zero-out padded tokens in K,V (queries stay)
        ...
        if self.redraw_interval > 0:
            ...
            if self._calls % self.redraw_interval == 0: self.proj_matrix = None
...
class PerformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, m_features, mlp_ratio=4.0, drop=0.0, attn_drop=0.0,
        ...
        self.attn = PerformerAttention(dim, num_heads, m_features, qkv_bias, attn_drop, drop, redraw_interval)
...
class TextPerformer(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor, attention_mask: Optional[torch.Tensor] = None):
        ...
        if attention_mask is not None:
            attn_mask = attention_mask[:, :, None, None].to(x.dtype)  # (B,N,1,1)
