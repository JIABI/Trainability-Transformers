...
class LSHBlockAttention(nn.Module):
    ...
    1) Hash tokens into n_buckets via random projection (winner-take-all).
    ...
    def _maybe_init_proj(self, C: int, n_buckets: int, device):
        ...
        if (self.hash_proj is None) or (self.hash_proj.shape != (C, n_buckets)) or (self.hash_proj.device != device):
            with torch.no_grad():
                self.hash_proj = torch.randn(C, n_buckets, device=device)
    ...
    def forward(self, x: torch.Tensor, attn_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        bucket_ids = hash_scores.argmax(dim=-1)  # (B,N)
        ...
        # 2) Pad to multiple of bucket_size
