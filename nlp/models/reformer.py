...
def masked_softmax(logits: torch.Tensor, key_mask: torch.Tensor, dim: int = -1):
    # logits: (..., Lq, Lk), key_mask: (..., Lk) with 1 for valid
    large_neg = torch.finfo(logits.dtype).min / 4
    mask_k = (~key_mask).unsqueeze(2).unsqueeze(2)
...
class LSHBlockAttention(nn.Module):
    ...
    1) Build content-based bucket ids via random projection (winner-take-all).
    2) Sort tokens by bucket id; split into equal blocks of size 'bucket_size'.
    3) Self-attend within each block (O(B * bucket_size^2)).
    Optional: n_hashes>1 currently uses the first hash (can be extended by repeating).
    """
    ...
    def __init__(self, dim: int, num_heads: int = 8, bucket_size: int = 64,
        ...
        # Random projection for hashing (winner-take-all)
    ...
    def forward(self, x: torch.Tensor, attn_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        """
        x: (B, N, C)
        attn_mask: (B, N) with 1 for valid, 0 for pad (optional)
        """
        B, N, C = x.shape
        device = x.device

        # 1) Hashing: content-based bucket ids
        # Use normalized token features (pre-q) for hashing
        x_norm = F.layer_norm(x, (C,))
        # choose number of buckets ~ N / bucket_size
        n_buckets = max(1, math.ceil(N / self.bucket_size))
        self._maybe_init_proj(C, n_buckets, device)
        # scores: (B,N,n_buckets); bucket id = argmax
        hash_scores = x_norm @ self.hash_proj  # (B,N,n_buckets)
        bucket_ids = hash_scores.argmax(dim=-1)  # (B,N) in [0..n_buckets-1]

        # stable sort by bucket id, then by position to break ties
        ...
        # permute tokens & masks
        def gather_N(t):
            return t.gather(1, perm.unsqueeze(-1).expand(-1, -1, t.shape[-1]))
        ...
        q = q.gather(1, perm.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, self.num_heads, self.head_dim))
        k = k.gather(1, perm.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, self.num_heads, self.head_dim))
        v = v.gather(1, perm.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, self.num_heads, self.head_dim))

        if attn_mask is None:
            attn_mask_sorted = torch.ones(B, N, device=device, dtype=torch.bool)
        else:
            attn_mask_sorted = attn_mask.gather(1, perm).bool()  # (B,N)

        # 2) Pad to multiple of bucket_size, reshape to blocks
        ...
        # 3) Within-block attention (vectorized over blocks/heads)
        # logits: (B,Bk,H,b,b)
        logits = torch.einsum('btqhd,btkhd->bthqk', q, k) / math.sqrt(self.head_dim)
        attn = masked_softmax(logits, key_mask)  # (B,Bk,H,b,b)
        attn = self.attn_drop(attn)
        ctx = torch.einsum('bthqk,btkhd->btqhd', attn, v)  # (B,Bk,b,H,Dh)

        # merge blocks, remove padding, invert permutation
        ctx = ctx.contiguous().view(B, n_blocks * bsz, self.num_heads, self.head_dim)  # (B,Npad,H,Dh)
        if pad > 0:
            ctx = ctx[:, :N, :, :]
...
def reformer_text_small(vocab_size: int, num_classes: int = 2, max_len: int = 1024,
    ...
    return TextReformer(vocab_size=vocab_size, num_classes=num_classes, max_len=max_len,
