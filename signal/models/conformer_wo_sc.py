...
class ConformerBlock1D(nn.Module):
    ...
    新增三个残差门：
      - alpha_mlp  : 作用于两侧 FFN 残差（各自再乘 0.5）
      - alpha_attn : 作用于 MHSA 残差
      - alpha_conv : 作用于卷积分支残差
    """
    def __init__(self, d_model: int, num_heads: int,
        ...
        # residual gates (buffers to allow .to(device) and no-grad toggling)
        self.register_buffer("alpha_mlp", torch.tensor(float(alpha_mlp)))
        self.register_buffer("alpha_attn", torch.tensor(float(alpha_attn)))
        self.register_buffer("alpha_conv", torch.tensor(float(alpha_conv)))

    def forward(self, x: torch.Tensor, key_padding_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        # FFN (left)
        x = x + 0.5 * self.alpha_mlp * self.droppath(self.ff1(self.ln1(x)))
        # MHSA
        x = x + self.alpha_attn * self.droppath(self.attn(self.ln2(x), key_padding_mask=key_padding_mask))
        # Conv
        x = x + self.alpha_conv * self.droppath(self.conv(x))
        # FFN (right)
        x = x + 0.5 * self.alpha_mlp * self.droppath(self.ff2(self.ln3(x)))
        return x
    ...
    def __init__(self,
        ...
        self.blocks = nn.ModuleList([
            ConformerBlock1D(
                ...
                drop_path=dpr[i],
                alpha_mlp=alpha_mlp, alpha_attn=alpha_attn, alpha_conv=alpha_conv
...
def conformer1d_small(num_classes: int = 5, in_chans: int = 1, d_model: int = 144,
                      ...
                      alpha_attn: float = 1.0,
                      alpha_mlp: float = 1.0,
                      alpha_conv: float = 1.0) -> Conformer1D:
    return Conformer1D(
        ...
        alpha_attn=alpha_attn, alpha_mlp=alpha_mlp, alpha_conv=alpha_conv
    )
