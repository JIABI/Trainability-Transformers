...
class LinformerAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # 可选：截断极端值避免 softmax 溢出（通常不触发，但更稳）
...
# -------------------------
# Transformer Block (Linformer attn)
# -------------------------
class Block(nn.Module):
    def __init__(self, dim, num_heads, seq_len, k_lin=None, mlp_ratio=4.0, qkv_bias=True,
                 drop=0.0, attn_drop=0.0, drop_path=0.0, share_kv=False, act_layer: str = "gelu"):
        super().__init__()
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            Block(embed_dim, num_heads, seq_len=self.seq_len, k_lin=k_lin, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias,
                  drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], share_kv=share_kv, act_layer=act_layer)
