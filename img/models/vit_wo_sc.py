...
class ResidualAlpha(nn.Module):
    def __init__(self, fn, alpha: float = 1.0):
        ...
        super().__init__()
        self.fn = fn
        # buffer: follows .to(device), saved in ckpt, no gradient by default
...
# -------------------------
# Transformer Block (with residual gates)
# -------------------------
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4.0, qkv_bias=True,
        ...
        # Wrap sub-branches with residual gates (include LN + sublayer + DropPath)
        self.resid_attn = ResidualAlpha(
            fn=lambda y: self.drop_path(self.attn(self.norm1(y))),
            alpha=alpha_attn
        )
...
# -------------------------
# Vision Transformer (with residual gates)
# -------------------------
...
# Factory functions
# -------------------------
def vit_b(patch_size=16, img_size=224, num_classes=80,
          drop_path_rate=0.1, global_pool="cls",
          # NEW: residual gates exposed here too
          alpha_attn: float = 1.0,
          alpha_mlp: float = 1.0):
...
def vit_b16(**kwargs):
    return vit_b(patch_size=16, **kwargs)


def vit_b32(**kwargs):
    return vit_b(patch_size=32, **kwargs)
